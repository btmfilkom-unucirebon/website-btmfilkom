...
async function fetchDataDepartemen() {
    try {
        ...
        const qDept = query(collection(db, "departemen_data"), orderBy("no_urut", "asc"));
        ...
        document.getElementById("container-departemen").innerHTML = `<p class="text-center text-red-500 col-span-full">Gagal memuat data. Cek koneksi internet.</p>`;
    }
}
...
function renderDepartemen(data) {
    ...
    data.forEach(dept => {
        ...
        if (dept.members && dept.members.length > 0) {
            ...
            membersHTML = dept.members.map(p => `
                <div class="flex-none w-full snap-center text-center px-4">
                    <div class="w-48 h-48 mx-auto rounded-3xl bg-white shadow-md overflow-hidden mb-4 border-4 border-white">
                        <img src="${p.foto}" class="w-full h-full object-cover" alt="${p.nama}" loading="lazy" />
                    </div>
        ...
        card.className = "dept-card bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden transition-all duration-500 cursor-pointer group my-5 md:my-0";
        // Pasang Event Listener langsung
        card.onclick = function() { window.toggleDept(this); };
