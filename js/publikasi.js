...
function isBeritaBaru(item) {
  ...
  return selisihHari <= BATAS_HARI_BARU;
}
...
async function fetchData() {
  try {
    ...
    allBerita = snapshotBerita.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
    }));
    ...
    allDokumentasi = snapshotDok.docs.map((doc) => ({
      id: doc.id,
      ...doc.data(),
    }));
...
function initPage() {
  ...
  const beritaTerbaru = allBerita.filter(
    (item) => isBeritaBaru(item) || item.isPinned === true,
  );
...
window.openFullListView = function (type) {
  ...
  if (type === "terbaru") {
    ...
    currentListData = allBerita.filter(
      (item) => isBeritaBaru(item) || item.isPinned === true,
    );
